...
import { IdCard, Loader2, Printer, Download } from "lucide-react";
    ...
    const handlePrint = () => {
        ...
        printWindow.document.write(`
            ...
            <style>
                ...
                .school { text-align: center; font-weight: bold; font-size: 18px; margin-bottom: 8px; }
                    ...
                    {isLoading ? (
                        ...
                        <>
                            <div className="flex gap-2">
                                <Button className="gap-2 bg-indigo-600 hover:bg-indigo-500" onClick={handlePrint}>
                                    <Printer className="h-4 w-4" /> Print All
                                </Button>
                            </div>
                            <div ref={printRef} className="space-y-6">
                                {cards.map((item: any) => (
                                    <div key={item.student._id} className="card rounded-xl border-2 border-gray-300 bg-white p-6 shadow-sm">
                                        <div className="school">{school?.schoolName ?? "School Name"}</div>
                                        <div className="exam">{item.exam?.title} • {item.exam?.startDate ? new Date(item.exam.startDate).toLocaleDateString() : ""}</div>
                                        <div className="flex justify-between gap-4">
                                            <div className="flex-1 space-y-1">
                                                <div className="row"><span className="label">Name:</span> {item.student?.firstName} {item.student?.lastName}</div>
                                                <div className="row"><span className="label">Admission No:</span> {item.student?.admissionNumber}</div>
                                                <div className="row"><span className="label">Class / Section:</span> {item.student?.class} - {item.student?.section}</div>
                                                <div className="row"><span className="label">Roll No:</span> {item.student?.rollNumber ?? "—"}</div>
                                                <div className="row"><span className="label">Father:</span> {item.student?.fatherName}</div>
                                            </div>
                                            <div className="photo rounded border border-gray-300 bg-gray-100 flex items-center justify-center text-xs text-gray-400">
                                                Photo
                                            </div>
                                        </div>
                                    </div>
